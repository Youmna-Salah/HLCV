function [features_per_image,nImgs,ifindex]=Invert_file_index(dataset_dir,vocabulary_tree)
...
for i= 1:nImgs
    ...
    for sdi = 1:sz %... each descriptor in image i
        ...
        if ~isempty(ifindex(index).images) &&~isempty(find(ifindex(index).images==i))
